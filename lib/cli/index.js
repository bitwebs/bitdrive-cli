...
class BitdriveServiceCommand extends Command {
  static keyFlag (info) {
    return flags.build({
  ...
  parsePath (mnt, path) {
    ...
    path = p.resolve(path)
    if (path.startsWith(p.join(mnt, 'Network'))) throw new Error('Bitdrive commands cannot be run on the Network directory.')
  ...
  async run () {
    try {
      this.client = new BitdriveServiceClient()
      await this.client.ready()
    } catch (err) {
      console.log('err:', err)
      console.error('Could not connect to the Bitdrive service. Is it running? Check with `bitdrive status`.')
      this.exit(1)
    }
  }
}
...
module.exports = BitdriveServiceCommand
